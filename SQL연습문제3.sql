...
acc_cid varchar2(14) not null, --처음에 char(14)로 하니까 account 기업고객은 14자 customer 기업고객은 12자라서 맞지 않아서 문제 25번이 기업은행이 조회되지 않았음
...
--실습 3-21
select tr_type,
case when tr_type = 1 then '입금'
     when tr_type = 2 then '출금'
     when tr_type = 0 then '조회' end as "거래종류",
...
from transaction
where tr_type = 1 or tr_type = 2 --tr_type in (1, 2)
group by tr_acc
having sum(tr_amount)>= 100000
order by sum(tr_amount) desc --order by 총거래금액 desc
...
select 
    *
from account a 
right join customer c on a.acc_cid = c.c_id;

--실습 3-26
select acc_no as 계좌번호, 
       acc_name as 계좌이름, 
...
--실습 3-27
select tr_acc, acc_cid, tr_type, tr_amount, tr_date 
from transaction t join account a on t.tr_acc = a.acc_no
order by tr_date;
...
where c.c_type = 1
group by a.acc_no, c.c_name; --거래내역이 왼쪽, 계좌가 오른쪽 거래내역이 없어도니까 오른쪽에 있는걸 모두 조회해야 하므로 right join
...
--tr_type =1인 경우 -->tr_amount를 합계
--tr_type !=1인 경우 --> 0으로 처리해라 
from 
account a
left join
transaction t on a.acc_no = t.tr_acc --내부join만 하면 김유신, 강감찬, 선덕여왕, 이순신밖에 안나옴
join customer c on a.acc_cid = c.c_id
where c_type = 1 
group by a.acc_no, a.acc_cid, a.acc_name, c.c_name, a.acc_balance
order by a.acc_no;
...
join(
    ...
    group by tr_acc)--타입이 1인 것을 계좌별로 묶어서 tr_acc와 total_deposit을 조회하라
    t on a.acc_no = t.tr_acc --
where t.total_deposit = (select max(sum_amount) 
                            from (select tr_acc, sum(tr_amount)as sum_amount
                            from transaction where tr_type = 1 group by tr_acc)
                            );-- 1.타입이 1인 것을 계좌별로 묶은것중에서 tr_acc와 tr_amount를 합한것을 골라라.
                              -- 2.그 중에서 합친금액이 가장 큰 것을 골라라 --> 입금액이 가장 많은 것
--그러면 입금액이 가장 많은 100만원인 선덕여왕의 이름과 계좌, total deposit, 주소가 조회된다.                    
